...
`{{"{{PAYLOAD_DATA_FIELD"}}}}` | Any data value sent in the JSON payload may be used as a variable, including custom data.  For example, if your JSON payload includes {% highlight json %} { "handler": { "key": "process-job", "id": 100}} {% endhighlight %}, then you can use the variables `{{"{{handler.key"}}}}` and `{{"{{handler.id"}}}}` in your notifications.
